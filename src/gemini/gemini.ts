...
export default async function geminiResponse(userMessage?: string, model: "welcome" | "default" = "default") {
  try {
    const restrictionsPromise = getRestrictionByTitle("restrictions")
    const messageModelPromise = model === "welcome" ? getRestrictionByTitle("greetingModel") : getRestrictionByTitle("standardModel")

    const today = new Date()
    const localeDateFormat = today.toLocaleDateString("pt-BR", { weekday: "long", year: "numeric", month: "long", day: "numeric" })

    const [responseModel, restrictions] = await Promise.all([messageModelPromise, restrictionsPromise])
